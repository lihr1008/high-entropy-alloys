...
data=pd.read_csv('O2_data\\spectra data.csv')
...
if __name__ == '__main__':
    ...
    for model in model_list:
        ...
        plt.scatter(X_tsne[:, 0], X_tsne[:, 1], marker='.', c=colors, s=30)
    ...
    plt.savefig(f'O2_data/tSNE-IR.png')
    # plt.show()

    with open('O2_data\\idx_gauss_IR.csv', 'w+', newline='') as csvfile:
        writer = csv.writer(csvfile)
        for row in idx_gauss:
            writer.writerow(row)

    free_energy = pd.read_csv(r'O2_data/free energy.csv')
    ...
    with open('O2_data\\idx_gauss_IR.csv','r') as csvfile:
    ...
    ######
    ddec6 = pd.read_csv(r'O2_data\DDEC06_charge.csv',encoding='gbk')
    ddec6 = ddec6.values
    ...
    fig.suptitle('O2_clustering results', fontsize=90)
    plt.savefig('O2_data\\O2_clustering_results.png',dpi=300)
    ...
    with pd.ExcelWriter(f"O2_data\\O2_results.xlsx") as writer:
    ...
    for i in range(4):
        ...
        with pd.ExcelWriter(f'O2_data\\spec_{i + 1}.xlsx') as writer:
