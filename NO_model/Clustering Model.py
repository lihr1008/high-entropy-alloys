...
data=pd.read_csv('NO_data\\spectra data.csv')
...
if __name__ == '__main__':
    ...
    plt.savefig(f'NO_data/tSNE-IR.png')
    # plt.show()

    with open('NO_data\\idx_gauss_IR.csv', 'w+', newline='') as csvfile:
        writer = csv.writer(csvfile)
        for row in idx_gauss:
            writer.writerow(row)

    free_energy = pd.read_csv(r'NO_data/free energy.csv')
    ...
    with open('NO_data\\idx_gauss_IR.csv','r') as csvfile:
    ...
    ######
    ddec6 = pd.read_csv(r'NO_data\DDEC06_charge.csv',encoding='gbk')
    ddec6 = ddec6.values
    ...
    fig.suptitle('NO_clustering results', fontsize=90)
    plt.savefig('NO_data\\NO_clustering_results.png',dpi=300)
    ...
    with pd.ExcelWriter(f"NO_data\\NO_results.xlsx") as writer:
    ...
    for i in range(4):
        ...
        with pd.ExcelWriter(f'NO_data\\spec_{i + 1}.xlsx') as writer:
